...
Health insurance provides protection from disruptive medical expenses
which might result from an illness, disability or accident. Based on the
health insurance coverage, insurance companies can either reimburse the
insured for incurred medical expenses or directly pay the care provider.
Thus, it is important for insurers to be able to forecast medical
expenses so they could accurately set yearly premiums that beneficiaries
have to pay to avail health insurance.

Hence, this project was aimed at developing a predictive model for
estimating medical expenses using linear regression method from a set of
predictor variables based on patient characteristics data.
...
For this analysis, a simulated dataset containing medical expenses of
patients in United States was used. This dataset was sourced from Kaggle
and which was created for the book “Machine Learning with R” (by Brett
...
The selected model - Model 4, which is an improved model that included
...
Based on the summary of Model 4, the variables with great predictive
power are `age2`, `children`, `smokeryes` and `bmi30:smokeryes`. A child
or dependent has an averaged increased cost of \$708 in medical expenses
per year. Furthermore, a smoker has an average increased cost of
\$13,500 per year, and an obese smoker will have an additional \$19,894
