...
p5_sex <- ggplot(data = insurance_df, aes(x= sex, fill = sex)) +
  geom_bar(width = 0.7, color = "dodgerblue1", fill = "dodgerblue3")+
  geom_text(data = . %>% 
              group_by(sex) %>% 
              tally() %>% 
...
p6_smoker <- ggplot(data = insurance_df, aes(x= smoker, fill = smoker)) +
  geom_bar(width = 0.7, color = "dodgerblue1", fill = "dodgerblue3")+
  geom_text(data = . %>% 
              group_by(smoker) %>% 
              tally() %>% 
...
# combined plots
ggarrange( p5_sex, p6_smoker, p7_region, ncol=3) #, labels = c ("F", "G"))
```
...
model.matrix(~0+., data = insurance_df_og) %>% 
  ...
  ggcorrplot(show.diag = F, type = "lower", lab = TRUE, lab_size = 3, colors = c("dodgerblue3", "white", "goldenrod2"),
